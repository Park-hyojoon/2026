...
def generate_ppt(songs_before, songs_after, template_path, output_path, worship_title, bible_title, bible_range, bible_body, sermon_title=""):
    ...
    try:
        with PowerPointManager() as ppt_mgr:
            ...
            # Check if the last slide is empty (has no text) to avoid double break slides
            last_slide = main_pres.Slides(main_pres.Slides.Count)
            has_text = False
            try:
                for shape in last_slide.Shapes:
                    if shape.HasTextFrame and shape.TextFrame.HasText:
                        if len(shape.TextFrame.TextRange.Text.strip()) > 0:
                            has_text = True
                            break
            except Exception:
                # If error reading shapes, assume has text to be safe and insert break
                has_text = True

            if has_text:
                print("Last slide has text. Inserting Break Slide...")
                main_pres.Slides(break_slide_index).Copy()
                # Paste at the end
                ...
                print("Last slide appears to be empty. Skipping Break Slide insertion.")
