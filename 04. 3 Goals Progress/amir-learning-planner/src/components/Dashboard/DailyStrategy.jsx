...
export default function DailyStrategy({ data }) {
    ...
    const getStrategy = () => {
        ...
        if (accountingProgress === 0) {
            return {
                title: "오늘의 핵심 전략: '기초 다지기'",
                instruction: "어제 회계 원리 개념을 마무리하셨네요. 오늘은 관련 기출문제 3개를 풀면서 실전 감각을 익히는 것이 우선순위입니다.",
                points: ["회계 순환 과정 복습", "기출문제 1~3번 풀이", "오답 노트 정리"],
                icon: Rocket,
        ...
        return {
            title: "공부 효율 극대화 전략",
            instruction: "현재 회계 공부 진도가 아주 좋습니다! 영어를 조금 더 보강하여 밸런스를 맞추는 날로 정해볼까요?",
            points: ["영어 핵심 구문 3번 말하기", "회계 심화 문제풀이", "AI 학습 로그 남기기"],
            icon: Zap,
